 ...
 * Implements hook_form_alter().
 */
function ncipq_form_alter(array &$form, array &$form_state = array(), $form_id = NULL) {
  if ($form_id) {
    switch ($form_id) {
      case 'search_form':
 ...
 */
function ncipq_search_form_wrapper($variables) {
 ...
 * @see hook_theme()
 */
function ncipq_theme(&$existing, $type, $theme, $path) {
  // Custom theme hooks:
...
function ncipq_preprocess_page(&$variables) {
...
function ncipq_links__system_main_menu($variables) {
  $html = "<div>\n";
  $html .= "  <ul>\n";  
...
/**
    ...
    */
function ncipq_preprocess_search_result(&$variables) {
